...
class ServerAPI:
    ...
    def build_request(self, service, command, *args):
        ...
        '''
        return f"{service}|{command}|" + "|".join(args)
    ...
    def get_info(self):
        ...
        '''
        return send_request(self.build_request("fetch", "info"))
    
    def login(self, email, password):
        '''
            This method is used to login to the server.
        '''
        return send_request(f"login|{email}|{password}")
    
    
if __name__ == '__main__':
    server = ServerAPI()
    # print(server.get_info())
    print(server.build_request("auth", "login", "ASd", "ASd"))